...
void main() {
  group('SupabaseEntryService timezone conversion', () {
    test('shift start/end times are converted to UTC before storing (Sweden winter, UTC+1)', () {
      ...
      expect(startUtc.hour, 7, reason: '08:00 Sweden time (UTC+1) should convert to 07:00 UTC');
      expect(startUtc.minute, 0);
      expect(endUtc.hour, 11, reason: '12:00 Sweden time (UTC+1) should convert to 11:00 UTC');
    ...
    test('shift start/end times are converted to UTC before storing (Sweden summer, UTC+2)', () {
      ...
      expect(startUtc.hour, 6, reason: '08:00 Sweden time (UTC+2) should convert to 06:00 UTC');
      expect(startUtc.minute, 0);
      expect(endUtc.hour, 10, reason: '12:00 Sweden time (UTC+2) should convert to 10:00 UTC');
      expect(endUtc.minute, 0);
    });
    ...
    test('UTC timestamps from DB are converted back to local for UI', () {
      ...
      expect(localStart.hour, 8, reason: '07:00 UTC should convert to 08:00 local (UTC+1)');
      expect(localStart.minute, 0);
      expect(localEnd.hour, 12, reason: '11:00 UTC should convert to 12:00 local (UTC+1)');
