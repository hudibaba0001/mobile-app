...
void main() {
  group('Daily Target Grouping Regression Test', () {
    test(
        'two atomic work entries same date: target applied ONCE, variance = 0 when worked = target',
        () {
    ...
    test('targetForDate called exactly once per unique date (not per entry)',
      ...
      expect(callCount, 1,
          reason:
              'targetForDate must be called ONCE per unique date, not per entry');
    ...
    test(
        'break logic with grouping: calculator sums workedMinutes (after breaks), not raw span',
        () {
      ...
      expect(summary.workedMinutes, 510,
          reason:
              'Calculator must sum workedMinutes (after breaks), not raw span. '
