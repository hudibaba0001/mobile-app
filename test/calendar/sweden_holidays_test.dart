  ...
  group('SwedenHolidayCalendar', () {
    test('Midsummer Day is Saturday and between Jun 20-26', () {
      ...
      for (int year = 2020; year <= 2030; year++) {
        ...
        final midsummer = holidays.firstWhere(
          ...
          orElse: () => throw Exception('Midsummer Day not found for year $year'),
        );
        
        // Must be Saturday (weekday 6)
        expect(midsummer.weekday, 6, reason: 'Midsummer Day must be Saturday in $year');
    ...
    test('All Saints Day is Saturday and between Oct 31-Nov 6', () {
      ...
      for (int year = 2020; year <= 2030; year++) {
        ...
        expect(allSaints, isNotNull, reason: 'All Saints Day not found for year $year');
        expect(allSaints!.weekday, 6, reason: 'All Saints Day must be Saturday in $year');
    ...
    test('Fixed-date holidays are correct', () {
      ...
      expect(holidays2025.contains(DateTime(2025, 1, 1)), isTrue); // New Year's Day
      ...
      expect(holidays2025.contains(DateTime(2025, 6, 6)), isTrue); // National Day
      expect(holidays2025.contains(DateTime(2025, 12, 25)), isTrue); // Christmas
      expect(holidays2025.contains(DateTime(2025, 12, 26)), isTrue); // Boxing Day
    });
    ...
    test('holidayName returns correct names', () {
      ...
      expect(calendar.holidayName(DateTime(2025, 1, 2)), isNull); // Not a holiday
    });
  });
}
